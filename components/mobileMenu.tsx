...
export const MobileMenu: React.FC<MobileMenuProps> = ({ isOpen, setIsExpanded }) => {
  ...
  return (
    <Container className={cn(`items-start flex-col justify-start text-white font-oswaldBold bg-basic pt-4 pb-6 relative`, isOpen ? 'flex' : 'hidden')}>
      <ul className=" relative flex flex-col justify-start items-start text-white  gap-y-[20px] pb-9 pt-4 px-2">
        {secondNav.map(item => {
